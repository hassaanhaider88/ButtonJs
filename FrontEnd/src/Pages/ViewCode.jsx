...
import ButtonsGridLoading from "../Components/ButtonsGridLoading";
...
const ViewCode = () => {
  ...
  useEffect(() => {
    ...
    const fetchSingleBtn = async () => {
      setLoading(true);
      try {
        const Res = await fetch(`${BGURI}/api/buttons/single-button/${id}`);
        const data = await Res.json();
        if (data.success) {
          setSelectBtn(data.data);
          setReactTailCode(data.data.ReactTailwindCode);
        } else {
  ...
  return (
    <section className="w-full text-white px-5 sm:px-10 md:px-20 py-7 min-h-screen ">
      {Loading ? (
        <ButtonsGridLoading />
      ) : (
        <>
