...
const SingleButtonOnUserProfile = ({
  buttonData,
    ...
    <div className="py-5">
      <div className="w-[350px] group cursor-pointer overflow-hidden relative h-[404px] mx-auto bg-[#5c5c5e31] rounded-[30px] z-[1] ">
        <div className="card__content absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          {IsLoginAndProfileViewerSame && (
            <span
              className={`rounded-3xl absolute -top-40 -right-20 ${
                buttonData.DeploymentStage === "Live"
                  ? "bg-yellow-300 py-1 px-4"
                  : "bg-green-300 py-1 px-2"
              }`}
            >
              {buttonData.DeploymentStage}
            </span>
          )}
      ...
      {
        buttonData.DeploymentStage === "Live" ? <div className="w-full flex justify-between items-center px-10 mt-2">
      ...
      </div> : <div className="w-full flex justify-between items-center px-10 mt-2">
        <span>{buttonData.buttonCategory}</span>
...
export default SingleButtonOnUserProfile;
